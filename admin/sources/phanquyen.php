...
function get_com(){
	global $d, $item_com;
	$d->reset();
	$sql = "select * from #_com where hienthi=1 order by stt,id desc";
	$d->query($sql);
	$item_com = $d->result_array();
}
...
function save_item(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data))			
				redirect("index.php?com=phanquyen&act=man".$urlcu);
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=phanquyen&act=man".$urlcu);
	}else{
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		} redirect("index.php?com=phanquyen&act=man".$urlcu);} else transfer("Không nhận được dữ liệu", "index.php?com=phanquyen&act=man".$urlcu);
}
