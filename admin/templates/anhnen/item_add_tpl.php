...
<form name="supplier" id="validate" class="form" action="index.php?com=anhnen&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
    <div class="widget">
        ...
        <div class="formRow">
            ...
            <div class="formRight">
                <select id="repeat" name="repeat" class="form-control">
                    <option value="no-repeat" <?php if($item['repea']=='no-repeat'){?>selected="selected"<?php }?>>Không lặp lại</option>
                    <option value="repeat" <?php if($item['repea']=='repeat'){?>selected="selected"<?php }?>>Lặp lại</option>
                    <option value="repeat-x" <?php if($item['repea']=='repeat-x'){?>selected="selected"<?php }?>>Chỉ lặp ngang</option>
                    <option value="repeat-y" <?php if($item['repea']=='repeat-y'){?>selected="selected"<?php }?>>Chỉ lặp dọc</option>
                </select>
            </div>
            <div class="clear"></div>    
        </div>
        ...
        <div class="formRow">
            <label>Cố định :</label>
            <div class="formRight">
                <input type="checkbox" name="fixed" id="fix_bg" value="fixed" <?=($item['fixed']=='fixed')?'checked="checked"':''?> />
        ...
        <div class="formRow">
          <label>Hiển thị hình : <img src="./images/question-button.png" alt="Chọn loại" class="icon_que tipS" original-title="Bỏ chọn để không hiển thị danh mục này ! "> </label>
          <div class="formRight">
