...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=hinhanh&act=man_photo&type=<?=$_REQUEST['type']?>&id_hinhanh=<?=$_REQUEST['id_hinhanh']?>"><span>Hình ảnh</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=hinhanh&act=add_photo&type=<?=$_REQUEST['type']?>&id_hinhanh=<?=$_REQUEST['id_hinhanh']?>'" />
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    ...
    <tbody>
    <form name="f" id="f" method="post"  action="index.php?com=hinhanh&act=savestt_photo<?php if($_REQUEST['id_hinhanh']!='') echo'&id_hinhanh='.$_REQUEST['id_hinhanh'];?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>">
    <?php for($i=0, $count=count($items); $i<$count; $i++){?>
       ...
       <td>
        ...
        <td class="title_name_data">
            <a href="index.php?com=hinhanh&act=edit_photo&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&id_hinhanh=<?=$_REQUEST['id_hinhanh']?>" class="tipS SC_bold">
			<?=$items[$i]['ten']?>
        ...
        <td class="actBtns">
            <a href="index.php?com=hinhanh&act=edit_photo&id=<?=$items[$i]['id']?><?php if($_REQUEST['id_hinhanh']!='') echo'&id_hinhanh='. $_REQUEST['id_hinhanh'];?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" title="" class="smallButton tipS" original-title="Sửa hình ảnh"><img src="./images/icons/dark/pencil.png" alt=""></a>
            <a href="index.php?com=hinhanh&act=delete_photo&id=<?=$items[$i]['id']?><?php if($_REQUEST['id_hinhanh']!='') echo'&id_hinhanh='. $_REQUEST['id_hinhanh'];?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa hình ảnh"><img src="./images/icons/dark/close.png" alt=""></a>        </td>
      </tr>
         <?php } ?>
    </form>
    </tbody>
