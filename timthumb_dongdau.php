...
if(strlen($src) && file_exists( $src ) ) {
	...
	show_image( $mime_type, $canvas, $quality, $cache_dir,$new_width);
...
function show_image ($mime_type, $image_resized, $quality, $cache_dir, $new_width) {
	...
	//$info = getimagesize($new_width);
	//dump($new_width);
	//Lấy kích thước hình và hình đóng dấu
	//if($new_width==250){$watermark = imagecreatefromfile('dong2.png');}
	//if($new_width==300){$watermark = imagecreatefromfile('dong3.png');}
	//else{$watermark = imagecreatefromfile('dong.png');}
	
	$watermark = imagecreatefromfile('dong.png');

	
 	if (!$watermark) die('Unable to open watermark');

	//imagesx($image_resized) Chiều rộng hình sản phảm
	//imagesx(imagesx($watermark)) Chiều rộng hình đóng dấu
	//imagesy($image_resized) Chiều cao hình sản phảm
	//imagesy($watermark) Chiều cao hình đóng dấu
 	$watermark_pos_x = imagesx($image_resized) - imagesx($watermark) - 8;
    $watermark_pos_y = imagesy($image_resized) - imagesy($watermark) - 10;

    imagecopy($image_resized, $watermark,  $watermark_pos_x, $watermark_pos_y, 0, 0,
    ...
    if(stristr( $mime_type, 'gif' ) ) {
        ...
        imagepng( $image_resized, $cache_file_name,9);
